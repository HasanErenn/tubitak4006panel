      ...
      <header className="bg-card/80 backdrop-blur-sm border-b border-gray-200/50 dark:border-gray-700/50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-3">
              {(session.user.role === 'ADMIN' || (session.user as any).role === 'TUBITAK_OKUL_YETKILISI') && (
                <Link
                  href="/admin"
                ...
                >
                  ...
                  {(session.user as any).role === 'TUBITAK_OKUL_YETKILISI' ? 'Okul Yönetimi' : 'Admin'}
                </Link>
              )}
              <ThemeToggle />
              <button
      ...
      <nav className="bg-card/60 backdrop-blur-sm border-b border-gray-200/50 dark:border-gray-700/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex space-x-1">
            {(session.user.role === 'ADMIN' || (session.user as any).role === 'TUBITAK_OKUL_YETKILISI') ? (
              // Admin/TUBİTAK Okul Yetkilisi Menüsü
              <Link
                href="/admin"
